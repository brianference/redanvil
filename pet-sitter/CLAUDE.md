# pet-sitter — build rules (inherited from RedAnvil corpus 1.0.0)

Read `DONE-CHECKLIST.md` in this directory before starting and before reporting anything finished. Nothing is done until every row of it has been measured and its evidence artifact opened. A spec, a prompt, or a plan is never evidence.
...
- **A first-time visitor gets a working app.** Verified with NOTHING forced -- no theme assigned, no storage primed, no route seeded, only an emulated operating-system preference. The default theme is LIGHT on a cold load whatever the OS says (a stored choice still wins, and the toggle still offers system) -- this was changed from following prefers-color-scheme on 2026-08-03 because a visitor on a dark phone got a dark first paint of an app whose intended default is light, and `cold_visitor.mjs` enforces it; the prose here said the opposite until 2026-08-09 and two apps were changed to match the wrong half before anyone noticed. The primary flow returns a real result for an input nobody seeded, and the console is clean on arrival. `scripts/cold-visitor.mjs` ships with the app and is scored by `fe-cold-visitor`.
