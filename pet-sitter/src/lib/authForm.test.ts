...
describe('formErrorMessage', () => {
  ...
  it('uses the fallback for unknown values', () => {
    expect(formErrorMessage('nope', en.auth.genericError)).toBe(en.auth.genericError);
  });
});
...
describe('safeInternalPath', () => {
  it('allows same-origin paths and rejects protocol-relative tricks', () => {
    ...
    expect(safeInternalPath('/sitters/sit-leslieville-01')).toBe('/sitters/sit-leslieville-01');
