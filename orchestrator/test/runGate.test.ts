...
const failCheck = (ruleId: string): Check => ({ ruleId, command: node, args: ['-e', 'process.exit(1)'] });

describe('runGate', () => {
  it('maps passing and failing checks to outcomes', async () => {
    const outcomes = await runGate(process.cwd(), [pass('u-typing-strict'), failCheck('u-sec-param-sql')]);
