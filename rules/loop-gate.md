...
- lg-role-deterministic-gate (blocker): tsc strict, eslint (`--max-warnings 0`), ruff/mypy where Python, vitest/pytest, `wrangler` build, runtime parity on the real Workers runtime, secret scan, SAST (semgrep / eslint-plugin-security), duplication scan, no committed binaries. Any tier-1 blocker fails the gate outright. Note: test COVERAGE is deliberately not listed. It was named here for months and measured by nothing, and a corpus line that implies a measurement nobody takes is worse than an honest gap — `u-test-presence`, `u-test-adequacy` and `u-test-behavioral` are what actually score the tests. Add a coverage threshold here only together with the tooling that enforces it.
