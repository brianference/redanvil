...
- lg-role-deterministic-gate (blocker): tsc strict, eslint (`--max-warnings 0`), ruff/mypy where Python, vitest/pytest, `wrangler` build, runtime parity on the real Workers runtime, secret scan, SAST (semgrep / eslint-plugin-security), duplication scan, no committed binaries. Any tier-1 blocker fails the gate outright. Test COVERAGE is now measured, and only because the tooling landed with it — the condition this line used to state. The scaffold ships `@vitest/coverage-v8` pinned to vitest's exact peer, a `test:coverage` script and a `json-summary` reporter, so a generated app emits `coverage/coverage-summary.json` and there is a real number to read. `u-test-presence` reads it against the diff (every changed source file must be exercised by something) and `u-test-coverage-ratchet` reads it against the app's own history (the tested fraction may only go up, and the bar cannot be edited down). There is deliberately still NO fixed percentage threshold: a global number invites writing tests that execute lines without asserting anything, and a floor that every app must clear is either too low to matter or too high for a young app to reach. The per-app ratchet is the honest form of the same requirement. What coverage still cannot see is the browser — components and pages are Playwright's surface, scored by `u-test-acceptance` and `u-test-feature-audit`, so do not read a coverage figure as though it covered the whole app.
...
- lg-shipped (blocker): an app is not finished until it is SHIPPED — in a GitHub repo, pushed, deployed to a production URL, proven to be serving the scored commit, AND measured at the finish line. Why: builds kept clearing 90+ and then sitting on a local disk with no remote and no URL, so "done" meant "the gate passed" rather than "a user can reach this build." Separately, repo+push+URL+hash alone let an unmeasured app be called shipped while the design audit never ran. A build that never ships is indistinguishable from one that was never built; a ship without a gate result is not done either. Checks (all five, fail closed): (1) the app directory is inside a git repository whose `origin` remote is a real GitHub URL; (2) current HEAD is pushed (`git rev-list origin/<branch>..HEAD` is empty — unpushed commits mean the remote does not contain what was gated); (3) a production URL is recorded (`.redanvil/claims.json` `deployUrl`) and returns HTTP 200; (4) the DEPLOYED bundle matches the LOCAL build — extract `assets/index-<hash>.js` from the fetched HTML and compare to the newest `dist/assets/index-*.js` (a wrangler success message is not proof; a matching hash is); (5) the app's own gate result meets the finish line (`results/<slug>.json` with finalScore >= threshold and zero rules with `passed === false`). Missing or sub-threshold results FAIL. N/A only when there is no wrangler.toml AND no deployUrl (not a deployable site). An app with a wrangler.toml and no remote is a FAIL, not n/a.
