 ...
 *
 * Per-route document title / description / OG / canonical was byte-identical
 * in app-builder and dashboard except the production origin. Parameterising
 * that one string lets both apps share the effect without fighting over which
 * host owns the canonical URL.
 */
import { useEffect } from 'react';

/** Per-route document metadata (title, description, path). */
 ...
 * @param siteOrigin - Absolute origin for the canonical URL (no trailing slash).
 */
export function useDocumentMeta(meta: DocumentMeta, siteOrigin: string): void {
  useEffect(() => {
  ...
  }, [meta.title, meta.description, meta.path, siteOrigin]);
}
