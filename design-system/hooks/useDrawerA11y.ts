 ...
 *
 * This hook was byte-identical in app-builder and dashboard (190 lines each),
 * counted twice by the cross-app duplication pass. Sharing it FAILED the first
 * time: with per-app node_modules the bundle carried two React instances and
 * every hook threw "Cannot read properties of null (reading 'useRef')" in the
 * browser, and resolve.dedupe did not fix it. It works now only because the
 * apps are npm workspaces and react is hoisted to a single copy at the root.
 */
