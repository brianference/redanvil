 ...
 */
function isNavActive(pathname: string, key: 'dashboard' | 'about'): boolean {
...
export function Page({ title, subtitle, breadcrumb, children }: PageProps): JSX.Element {
  ...
  const dashboardActive = isNavActive(location.pathname, 'dashboard');
  const aboutActive = isNavActive(location.pathname, 'about');
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: theme.space.sm, flexShrink: 0 }}>
            <nav className="ra-desktop-nav" aria-label={en.app.primaryNav}>
              <a href={APP_URL} className="ra-nav-link">
                {en.app.navBuilder}
              </a>
              <Link
                to="/"
                className={`ra-nav-link${dashboardActive ? ' is-active' : ''}`}
              ...
              <Link
                to="/about"
                className={`ra-nav-link${aboutActive ? ' is-active' : ''}`}
