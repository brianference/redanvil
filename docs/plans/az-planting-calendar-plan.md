...
## Resolved locally after this plan was written

Two items the cloud session could not settle, because its network policy reached
GitHub only. Both are resolved here with evidence, and both change the plan.

### The open item is closed: AZ1005 is HALF-month, 24 columns

The plan left granularity unresolved and had the schema carry a per-window
`source_granularity` against the possibility of whole-month columns. It does not
need to. The PDF's own header row, extracted with `pypdf` from a local copy:
...
Twelve months, each with a `1` and a `15`, counted programmatically: **24**. So
the 24-column grid the prompt asked for matches the source exactly, and
`source_granularity` can be dropped for AZ1005 rather than encoding a doubt that
turned out to be unfounded.

### The dataset does not need to be pasted

The plan asks the user to paste the crop table because
`extension.arizona.edu` returned `CONNECT tunnel failed, 403`. That is a
restriction of the cloud environment, not of the work: the PDF is already
downloaded locally and `pypdf` extracts its text. Part C's Slice 2 is therefore
not gated on a human paste.

What IS still unsolved is column ALIGNMENT, and it is narrower than "get the
data". Measured on the local copy:

- the `T`/`S` markers extract cleanly with coordinates (304 on page 2, 273 on page 3)
- page 2's vertical rules give 26 boundaries where 24 columns need 25
- page 3 is missing a rule entirely: its gaps run 14.9pt except one jump of 25.7pt
- the month header exists in the content stream but not as positioned characters,
  so there is no in-PDF anchor binding column x-ranges to months

Binning by arithmetic is what produces a calendar that is confidently half a
month off, which is why the extractor in `scratch/az/extract.py` fails closed
rather than emitting. With the 24-column count now confirmed, the remaining task
is to anchor the first column, then verify a known row against the rendered page
before any of it seeds a migration.

### Both still stand

The plan's Part A findings are independent of all of the above and are the
durable half of this work. RA-163 removed `calendar` from the scheduling
patterns but added nothing, so this prompt now matches no capability kind at
all — a quieter wrong answer. The `reference` kind (A2), the fail-closed empty
entities (A1), and above all the prompt-fidelity check (A7) are what stop the
next spec from grading itself 12/12 while describing a different product.
