...
describe('estimate', () => {
  ...
  it('raises confidence when optional scope signals are filled in', () => {
    ...
    expect(estimate({ features: 1, hasAuth: false, entities: 0, scopeSignals: 3 }).confidence).toBe(
      'medium'
    );
    // Mid weight becomes high with rich scope
    expect(estimate({ features: 4, hasAuth: true, entities: 3, scopeSignals: 3 }).confidence).toBe(
      'high'
    );
  });
});
